...
public class DelComputerModule extends Page{
	
	public DelComputerModule(WebDriver driver, Log log) throws InvalidFormatException, IOException {
		super(driver, log);
	}	
	@Override
	protected void load() {}
	...
	public boolean deleteComp(String AccNo, String Testname) throws Exception  {
	    ...
	    if (elementExistCssSelect(properties.getProperty("deleteComputer"), 5)) {    
			WebElement btnClick = driver.findElement(By.cssSelector(properties.getProperty("deleteComputer")));
			((JavascriptExecutor) driver).executeScript("arguments[0].click()", btnClick);
			
			Thread.sleep(1000);
			log.logLine(Testname, false, "Clicking on Delete this computer is successfull");
 		} else {
 			log.logLine(Testname, true, "Clicking on Delete this computer is unsuccessfull");
 			throw new Exception("Clicking on Delete this computer is unsuccessfull");
	 	}		    
	    
	    
	    Thread.sleep(400);
	    if (elementExistCssSelect(properties.getProperty("ConfMessage"), 5)) {	    	
	    	log.logLine(Testname, false, "Deletion of Existing computer completed successfully");	
	    	return true;
	    } else {
	    	log.logLine(Testname, true, "Deletion of Existing computer is failed");		
	    	throw new Exception("Deletion of Existing computer is failed");		
	    }	    
