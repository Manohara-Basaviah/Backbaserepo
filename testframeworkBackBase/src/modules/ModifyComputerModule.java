...
public class ModifyComputerModule extends Page{
	
	public ModifyComputerModule(WebDriver driver, Log log) throws InvalidFormatException, IOException {
		super(driver, log);
	}	
	@Override
	protected void load() {}
	...
	public boolean modifyComputer(String AccNo, String Testname, String MultiFilesupld) throws Exception  {
	    ...
	    String UpdateCompName = test.readColumnData("UpdateCompName", sheetname);			    
	    
		if (elementExistXpath("//*[contains(text(),'"+UpdateCompName+"')]", 5)) {	  
			WebElement dropdownlink = driver.findElement(By.xpath("//*[contains(text(),'"+UpdateCompName+"')]"));
			dropdownlink.click();
			log.logLine(Testname, false, "Selected Company as :" +UpdateCompName);			
		}else {
			log.logLine(Testname, true,"Unable to update the company name in the page");
			throw new Exception("Unable to update the company name in the page");
		}
		    
	    if (elementExistCssSelect(properties.getProperty("saveComputer"), 5)) {    
			WebElement btnClick = driver.findElement(By.cssSelector(properties.getProperty("saveComputer")));
			((JavascriptExecutor) driver).executeScript("arguments[0].click()", btnClick);
			
			Thread.sleep(1000);
			log.logLine(Testname, false, "Clicking on Save this computer is successfull");
 		} else {
 			log.logLine(Testname, true, "Clicking on Save this computer is unsuccessfull");
 			throw new Exception("Clicking on Save this computer is unsuccessfull");
	 	}		    
	    
	    
	    Thread.sleep(400);
	    if (elementExistCssSelect(properties.getProperty("ConfMessage"), 5)) {	    	
	    	log.logLine(Testname, false, "Modification of Existing computer completed successful");	
	    	return true;
	    } else {
	    	log.logLine(Testname, true, "Modification of Existing computer is failed");		
	    	throw new Exception("Modification of Existing computer is failed");		
	    }	    
